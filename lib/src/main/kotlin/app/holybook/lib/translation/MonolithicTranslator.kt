...
/**
 * A monolithic implementation of the Translator interface that uses Google's Gemini AI to translate
 * text with support for authoritative reference translations.
 *
 ...
 */
class MonolithicTranslator @Inject constructor(private val modelConfiguration: ModelConfiguration) :
  Translator {
  ...
  override fun translate(request: TranslationModelRequest): TranslationModelResponse {
    ...
    val systemInstruction =
      this::class.java.classLoader.getResourceAsStream("system_prompt.txt")?.use { inputStream ->
    ...
    val response = Json.decodeFromString<TranslationModelResponse>(modelResponseText)
